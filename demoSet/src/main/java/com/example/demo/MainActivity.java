...
import com.example.demo.fragment.DialogFragment;
import com.example.demo.fragment.DialogFragment_;
import com.example.demo.fragment.DrawingFragment_;
import com.example.demo.fragment.FrameAnimationFragment_;
import com.example.demo.fragment.HeartRateMonitorFragment;
import com.example.demo.fragment.HeartRateMonitorFragment_;
import com.example.demo.fragment.ImageDemoFragment_;
import com.example.demo.fragment.MonitorFragment_;
import com.example.demo.fragment.MultiThreadDownloadFragment_;
import com.example.demo.fragment.MusicPlayerFragment_;
import com.example.demo.fragment.PhoneMonitorFragment;
import com.example.demo.fragment.PhoneMonitorFragment_;
...
,PhoneMonitorFragment.GetPathAble{
    ...
    private String phoneMonitorTag="phoneMonitor";
    static {
        initData();
    }
...
//    RecyclerView rv;
    ...
    private void selected14ChildItem(int childPosition) {
        switch (childPosition) {
            case 0:
                getFragment(new PhoneMonitorFragment_(),this.phoneMonitorTag);
                break;
            case 1:
                Toast.makeText(this, "demo尚未添加", Toast.LENGTH_SHORT).show();
                break;
    ...
    private void selected13ChildItem(int childPosition) {
        switch (childPosition){
            case 0:
                getFragment(new MonitorFragment_());
                break;
            case 1:
                getFragment(new HeartRateMonitorFragment_());
                break;
            case 2:
                getFragment(new FrameAnimationFragment_());
                break;
            case 3:
                getFragment(new DialogFragment_());
                break;
            case 4:
                Toast.makeText(this, "demo尚未添加", Toast.LENGTH_SHORT).show();
                break;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode){
            case  MainActivity.CONTACT_REQUEST_CODE:
                returnUserNumber(data);
                break;
            case  MainActivity.PICKFILE_REQUEST_CODE:
                returnPath(data.getDataString());
        }
    }

    private void returnPath(String dataString) {
        PhoneMonitorFragment_ phoneMonitorFragment = (PhoneMonitorFragment_) manager.findFragmentByTag(this.phoneMonitorTag);
        phoneMonitorFragment.setPath(dataString);
    }


    private void returnUserNumber(Intent data) {
...
//        intent.setType("file/");
//        startActivityForResult(intent, MainActivity.PICKFILE_REQUEST_CODE);
...
//                return !pathname.getPath().matches("(.)*/\\.(.)*");
            }
        });
        builder.setItems(list, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if(file.isDirectory()){
                    ...
                    Toast.makeText(MainActivity.this, "请选择一个文件夹", Toast.LENGTH_SHORT).show();
                }
            }
        }).setPositiveButton("确定", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                PhoneMonitorFragment_ fragment = (PhoneMonitorFragment_) manager.findFragmentByTag(MainActivity.this.phoneMonitorTag);
